...
export default function Calendar({
  ...
  const displayText = displayDate ? format(displayDate, 'MMM dd, yyyy') : placeholder;
    ...
    <div className="relative w-full" ref={wrapperRef}>
      <button
        ...
        className={`flex items-center justify-between px-3 py-2 text-sm hover:border-gray-300 focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:text-gray-400 disabled:cursor-not-allowed ${
          className || 'w-full bg-white border border-[#E0E0E0] rounded-sm disabled:bg-gray-50'
        }`}
      >
        <span className={displayDate ? 'text-[#242424]' : 'text-[#707070]'}>{displayText}</span>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 z-50 mt-2 p-4 bg-white border border-[#E4E4E7] rounded-[12px] shadow-lg">
          ...
          <div className="flex gap-2 mb-4">
            ...
            />
            <button
              onClick={() => setDisplayMonth(new Date(currentYear, currentMonth - 1))}
          ...
          <DayPicker
            ...
            classNames={{
              ...
              head_cell: 'w-8 h-8 text-center text-xs font-medium text-[#616161] uppercase',
              ...
              day_outside: 'text-[#A1A1A1] opacity-50 aria-disabled:cursor-not-allowed',
              day_disabled: 'text-[#A1A1A1] cursor-not-allowed opacity-50',
