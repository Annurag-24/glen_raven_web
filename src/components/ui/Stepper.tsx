    ...
    return (
        <StepItemContext.Provider value={{ step, state, isDisabled: disabled, isLoading }}>
            <div
                ...
                className={cn(
                    'group/step flex  justify-center group-data-[orientation=horizontal]/stepper-nav:flex-row group-data-[orientation=vertical]/stepper-nav:flex-col not-last:flex-1 items-baseline',
                    className,
                )}
                data-state={state}
...
function StepperIndicator({ children, className }: React.ComponentProps<'div'>) {
    ...
    return (
        <div
            ...
            data-state={state}
            className={cn(
                'relative flex items-center overflow-hidden justify-center size-6 shrink-0 border-background bg-accent text-accent-foreground rounded-full text-xs ',
                // Allow custom colors via className - default colors can be overridden
                'data-[state=completed]:bg-[#036FED] data-[state=completed]:text-white data-[state=completed]:border-[#036FED]',
                'data-[state=active]:bg-[#0B6936] data-[state=active]:text-white data-[state=active]:border-[#0B6936]',
                'data-[state=inactive]:bg-white data-[state=inactive]:text-gray-400 data-[state=inactive]:border-gray-300',
                className,
            )}
        >
...
function StepperSeparator({ className }: React.ComponentProps<'div'>) {
    ...
    return (
        <div
            ...
            data-state={state}
            className={cn(
                'm-0.5 rounded-full bg-gray-200 group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5 group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1',
                // Allow custom colors - default to gray, can be overridden via className
                'group-data-[state=completed]/step:bg-[#036FED]',
                className,
            )}
        />
    );
...
function StepperNav({ children, className }: React.ComponentProps<'nav'>) {
    ...
    return (
        <nav
            ...
            data-orientation={orientation}
            className={cn(
                'group/stepper-nav inline-flex data-[orientation=horizontal]:w-full data-[orientation=horizontal]:flex-row data-[orientation=vertical]:flex-co ',
                className,
            )}
        >
            {children}
