...
import { actionItems, drawerMenuItems } from "@/constants/navigation";
...
const LeftSidebarDrawer: React.FC<ILeftSidebarDrawerProps> = ({
  ...
  return (
    <div className="fixed top-14 left-0 w-64 h-full bg-white shadow-lg z-50 border-r border-slate-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            {actionItems.map((item, index) => (
              ...
              >
                ...
                <div className="justify-start text-gray-500 text-[10px] font-medium group-hover:text-tertiary">
                  {item.name}
                </div>
              </Link>
            ))}
      ...
      >
        <img src={ChevronRightIcon} alt="Menu" className="w-4 h-4 rotate-180" />
      </button>
    </div>
  );
};
