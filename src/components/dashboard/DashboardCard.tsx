  ...
  return (
    ...
    >
      <div className="self-stretch inline-flex justify-between items-center">
        <p className="justify-center text-Grey-600 text-lg font-bold leading-7">{title}</p>
        {showViewAll && (
          <Link
            to={viewAllLink || '#'}
...
export const CardPrimaryText: React.FC<ITextProps> = ({
  ...
  return (
    <p className={cn([`${color} text-base font-semibold leading-6`, className])}>{children}</p>
  );
};
...
export const CardSecondaryText: React.FC<ITextProps> = ({
  ...
  return <p className={cn([`${color} text-sm font-normal leading-5`, className])}>{children}</p>;
};
...
export const CardTableHeader: React.FC<ITableHeaderProps> = ({ children, className }) => {
  return (
...
export const CardTableBody: React.FC<ITableProps> = ({ children, className }) => {
  return <tbody className={className}>{children}</tbody>;
};

export const CardTableRow: React.FC<ITableRowProps> = ({ children, className, isLast = false }) => {
  return <tr className={cn([!isLast && 'border-b border-gray-200', className])}>{children}</tr>;
};
