...
export type FilterField = {
  key: string;
  label: string;
  type: 'select' | 'multiselect' | 'text' | 'input' | 'date' | 'number' | 'select-component';
  placeholder?: string;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {fields.map((f) => (
          <div key={f.key} className="space-y-1">
            ...
            {(f.type === 'input' || f.type === 'text' || f.type === 'number') && (
              <input
            ...
            {f.type === 'date' && (
              <Calendar
                value={values[f.key] ? new Date(values[f.key] as string) : undefined}
                onChange={(date) => {
                  if (!date) {
                    setValue(f.key, '');
                    return;
