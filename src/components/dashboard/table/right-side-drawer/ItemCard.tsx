  ...
  return (
    <div className="w-full p-2.5 bg-white rounded-lg outline-1 -outline-offset-1 outline-gray-200 flex flex-col gap-3">
      <div className="flex items-start gap-2.5">
        <img className="w-20 h-20 rounded-lg shrink-0" src={item.imageUrl} alt={item.description} />
        <div className="flex-1 flex flex-col gap-1">
          <div className="flex items-center flex-wrap gap-1">
            <span className="text-gray-800 text-sm font-semibold leading-5">Tracking ID: </span>
            <span className="text-tertiary text-sm font-semibold leading-5">{item.trackingId}</span>
          </div>
          <div className="text-gray-800 text-xs font-medium leading-5">{item.description}</div>
          <div className="text-gray-500 text-xs font-medium leading-4">{item.details}</div>
        </div>
      </div>
      <div className="flex justify-between items-center">
        ...
        <div className="text-gray-800 text-xs font-semibold leading-5">{item.price}</div>
      </div>
    </div>
  );
};
