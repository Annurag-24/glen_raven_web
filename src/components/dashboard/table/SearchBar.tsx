...
import { Search, RotateCw, Filter, Settings, Upload, X, Check, Trash2 } from 'lucide-react';
...
> = ({ children, title, className, ...props }) => {
  const defaultClasses = 'inline-flex items-center justify-center rounded-[6px] cursor-pointer';
...
const SearchBar: React.FC<SearchBarProps> = ({
  ...
  React.useEffect(() => {
    if (!isControlled && typeof defaultValue !== 'undefined') setInternal(defaultValue);
    ...
    <div className="w-full bg-white px-3 py-2 flex items-center justify-between border-b border-[#E5E6F0]">
      ...
      <div className="flex items-center gap-2 w-full justify-end">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {showSettings && (
            <IconButton title="Settings" aria-label="settings" onClick={onSettings}>
          ...
          {customButtons
            ...
            .map((b) => (
              <IconButton key={b.key} title={b.title} className={b.className} onClick={b.onClick}>
                {b.icon}
              </IconButton>
            ))}
