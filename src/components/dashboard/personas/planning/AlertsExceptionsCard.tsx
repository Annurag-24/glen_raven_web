...
export default function AlertsExceptionsCard() {
  return (
    <DashboardCard title="Alerts & Exceptions" showViewAll={false} wrapperClassName="w-full">
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap gap-2">
          {tags.map((t) => (
            <Badge key={t.label} variant={t.variant as BadgeVariant} title={t.label} />
          ))}
        </div>
            ...
            {data.map((d, idx) => (
              ...
              >
                <div>
                  ...
                  <CardSecondaryText>{d.detail}</CardSecondaryText>
                </div>
