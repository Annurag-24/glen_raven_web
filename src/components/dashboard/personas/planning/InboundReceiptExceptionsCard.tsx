...
export default function InboundReceiptExceptionsCard() {
  return (
    ...
    >
      <CardTable>
        <CardTableHeader>
          <CardTableHeaderCell width="w-40">PO Number</CardTableHeaderCell>
        ...
        <CardTableBody>
          {data.map((row, idx) => (
            <CardTableRow key={row.po} isLast={idx === data.length - 1}>
              <CardTableCellPrimary width="w-40">{row.po}</CardTableCellPrimary>
              <CardTableCell>{row.ordered}</CardTableCell>
              <CardTableCell>{row.received}</CardTableCell>
              <CardTableCell>
                <Badge title={row.issue} variant={mapIssueToVariant(row.issue)} />
              </CardTableCell>
            </CardTableRow>
          ))}
        </CardTableBody>
