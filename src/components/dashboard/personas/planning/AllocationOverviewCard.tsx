...
const renderAllocated = (value: number, _supply: string, demand: string) => {
  ...
  return <span className={`${cls} font-medium`}>{value.toLocaleString()}</span>;
};
  ...
  return (
    ...
    >
      <CardTable>
        ...
        <CardTableBody>
          {data.map((row, idx) => (
            <CardTableRow key={row.sku} isLast={idx === data.length - 1}>
              <CardTableCellPrimary width="w-40">{row.sku}</CardTableCellPrimary>
              <CardTableCell>{row.supply}</CardTableCell>
              <CardTableCell>{row.demand}</CardTableCell>
              <CardTableCell>
                {renderAllocated(row.allocated, row.supply, row.demand)}
              </CardTableCell>
            </CardTableRow>
          ))}
        </CardTableBody>
